...
const encryptFile = async encrypt_request => {
    ...
    const AESKEY = encrypt_request.AESKEY; // Clave de cifrado

    console.log(AESKEY)


    const algorithm = 'aes-256-cbc'; // Algoritmo de cifrado
    const key = Buffer.from(AESKEY, 'hex'); // Reemplaza con tu clave de 32 bytes
...
const decryptFile = async decrypt_request => {
    ...
    const AESKEY = path.join(encrypt_request.AESKEY); // Clave de cifrado

    // Configuración
    const algorithm = 'aes-256-cbc'; // Algoritmo de cifrado
    const key = Buffer.from(AESKEY, 'hex'); // Reemplaza con la clave generada en formato hexadecimal
